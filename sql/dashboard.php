...
$myXP=0; try{ $s=$pdo->prepare("SELECT COALESCE(SUM(points),0) FROM user_xp WHERE user_id=?"); $s->execute([$userId]); $myXP=(int)$s->fetchColumn(); }catch(Throwable $e){}
...
try{
  $leadersAll=$pdo->query("
    SELECT ux.user_id, IFNULL(u.username, CONCAT('User #', ux.user_id)) AS display_name, SUM(ux.points) AS total_points
...
<?php include __DIR__ . '/inc/app_topbar.php'; ?><!-- render the navbar -->
...
?>
  <div style="background:rgba(255,255,255,0.08);border-radius:14px;box-shadow:0 2px 12px rgba(0,0,0,0.12);overflow:hidden;">
    ...
    <div style="padding:18px 16px;">
      ...
      <span style="float:right;color:#a8b4d7;font-size:.9rem;"><?= (int)$module['duration_minutes'] ?> min</span>
    ...
    <div class="rs-body">
      ...
      <div class="rs-section">
        ...
        <ul>
          ...
          <li>(Optional) Take a post-test for extra XP).</li>
        </ul>
      </div>
      <div class="rs-section">
        <h4>Data & privacy</h4>
...
document.addEventListener('DOMContentLoaded', function() {
  var avatar = document.getElementById('pgAvatar');
  var dropdown = document.getElementById('pgUserDropdown');
  if (avatar && dropdown) {
...
window.fetch = async (...args) => {
  ...
  try {
    ...
    if (j && j.ok){
      ...
      if (awarded) {
        // Small microtask delay to not block current UI updates
        Promise.resolve().then(refreshHud);
      }
    }
...
(() => {
  ...
  function updateTimer(){
    ...
    serverNow += 1;
    if (left <= 0){
      statusEl.innerHTML = '⌛ <span style="color:#fbbf24">Time expired on server</span>.';
  ...
  document.getElementById('smSubmit').addEventListener('click', async ()=>{
    ...
    try{
      ...
      // Prefer total_xp from backend to avoid drift; fallback to adding delta
      try {
...
(() => {
  ...
  const anyResearchModalOpen=()=>[mInfo,mConsent,mPre,mPost,mExit].some(el=>el && !el.hidden);
  ...
  function showExit(kind){
    if(exitMsg){
      ...
      switch(kind){
        case 'decline':
          html = `<p>Thanks for considering it.</p><p>You chose not to participate at this time. Participation requires consent.</p>`;
          break;
        case 'no-consent':
          html = `<p>Thanks for reviewing the form.</p><p>To participate, you must provide consent. You can return anytime if you change your mind.</p>`;
          break;
        case 'under-18':
          html = `<p>Thank you for your interest.</p><p>Unfortunately, you must be 18 years or older to take part in this study.</p>`;
          break;
        case 'pre-ineligible':
        case 'post-ineligible':
          html = `<p>Thanks for completing the ${kind==='pre-ineligible'?'pre':'post'}-test screen.</p><p>Participation requires confirming you are 18+ and that you consent.</p>`;
          break;
        default:
  ...
  let preStartedMs=0;
  function collectPre(){
    const pickRadio = (n)=>{ const r=[...document.querySelectorAll(`input[name="${n}"][type=radio]`)]; const x=r.find(i=>i.checked); return x?x.value:null; };
    const out={}; out['elig_18']=pickRadio('elig_18'); out['elig_consent']=pickRadio('elig_consent');
    for(let i=1;i<=15;i++) out['q'+i]=pickRadio('q'+i); return out;
  }
  if(preSubmit) preSubmit.addEventListener('click', async ()=>{
    ...
    try{
      ...
      preMsg.textContent=`Saved! Score ${data.score}/${data.total}. +${data.awarded_xp} XP`; setTimeout(()=>location.reload(),700);
...
<script>
(() => {
  const postBtn = document.getElementById('postOpenBtn');
  ...
  function collectPost() {
    ...
    const pickMany = n => {
      ...
      return els.length ? els.map(x=>x.value) : null;
    };
  ...
  if (postBtn) postBtn.addEventListener('click',()=>{ postMsg.textContent=''; show(mPost); setPageLocked(true); });
  ...
  if (postSubmit) {
    ...
    postSubmit.addEventListener('click', async ()=>{
      ...
      try {
        ...
        const keys = Object.keys(a).filter(k => k !== 'pq_elig_18' && k !== 'pq_consent');
        if (keys.length === 0) {
          postMsg.textContent = 'Please answer at least one question.';
